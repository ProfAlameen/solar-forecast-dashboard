...
# SHAP - KernelExplainer (slow but works for any model)
import shap
from shap import KernelExplainer

# Flatten sequences for compatibility
X_flat = X.reshape((X.shape[0], -1))
model_wrapper = lambda x: model.predict(x.reshape((-1, lookback, len(features)))).flatten()

# Sample a subset for SHAP analysis
X_sample = X_flat[:20]
explainer = KernelExplainer(model_wrapper, X_sample)
shap_values = explainer.shap_values(X_sample, nsamples=100)

shap.summary_plot(shap_values, X_sample, feature_names=[f"{f}_t{t}" for t in range(lookback) for f in features], show=False)
plt.tight_layout()
...
print("✅ SHAP (kernel) report generated at outputs/final_report.html")
